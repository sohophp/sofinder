...
final readonly class SharedAssetCatalog implements AssetCatalogInterface
{
    ...
    public function register(string $workspace, string $resource, Entry $entry): AssetRecord
    {
        ...
        $this->mutate(function (array &$data) use ($workspace, $resource, $entry, &$record): void {
            ...
            $value = is_array($data['assets'][$id] ?? null) ? $data['assets'][$id] : ['alt' => null, 'title' => null, 'tags' => [], 'metadataVersion' => 1, 'updatedAt' => time()];
    ...
    {
        $record = null;
        $this->mutate(function (array &$data) use ($assetId, $alt, $title, $tags, $expectedVersion, &$record): void {
            ...
            $data['assets'][$assetId]['alt'] = $alt; $data['assets'][$assetId]['title'] = $title; $data['assets'][$assetId]['tags'] = $tags; $data['assets'][$assetId]['metadataVersion'] = $current + 1; $data['assets'][$assetId]['updatedAt'] = time(); $record = $this->record($assetId, $data['assets'][$assetId]);
        });
