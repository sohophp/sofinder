...
final readonly class AssetAccessSessionManager
{
     ...
     */
    public function create(array $assetIds, ?int $ttl = null): array
    {
        ...
        $workspace = $this->workspaces->current(); $assets = [];
        foreach ($assetIds as $assetId) {
            ...
            $this->workspaces->assertResource($record->resource); $resource = $this->resources->get($record->resource); if ($resource->resource->deliveryMode !== 'proxy') throw new SoFinderException('Access sessions only accept private proxy assets.', 'asset_access_session_public_asset', 422);
