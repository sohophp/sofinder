...
final readonly class AssetReferenceFactory
{
    public function __construct(
        ...
        /** @var list<int> */ private array $variantWidths = [],
        /** @var list<string> */ private array $variantFormats = [],
    ) {
    }
    ...
    {
        if ($entry->directory) throw new \InvalidArgumentException('Asset references can only be created for files.');
        ...
        if ($dimensions === null && $entry->mimeType !== null && str_starts_with($entry->mimeType, 'image/') && $this->images !== null) {
            try { $dimensions = $this->images->info($resource, $entry->path); } catch (\Throwable) { $dimensions = null; }
        }
        ...
        if ($embeddable && $this->variantsEnabled && $dimensions !== null && $entry->mimeType !== null && str_starts_with($entry->mimeType, 'image/')) {
            $format = $this->images?->preferredVariantFormat($entry->mimeType, $this->variantFormats) ?? 'original'; $mime = $format === 'original' ? $entry->mimeType : 'image/' . $format;
            foreach ($this->variantWidths as $width) {
                if ($width > (int) $dimensions['width']) continue;
                $height = max(1, (int) round((int) $dimensions['height'] * ($width / (int) $dimensions['width'])));
                $variants[] = ['width' => $width, 'height' => $height, 'mimeType' => $mime, 'url' => $this->router->generate('sofinder_image_variant', ['resource' => $resource, 'path' => $entry->path, 'width' => $width, 'format' => $format, 'v' => $entry->modifiedAt . '-' . $entry->size], UrlGeneratorInterface::ABSOLUTE_PATH)];
            }
        }
        return [
