...
"$repository_root/scripts/php-bin.sh" -r 'require "vendor/autoload.php"; exit(class_exists("SohoPHP\\SoFinder\\Http\\Action\\MetricsAction") && class_exists("SohoPHP\\SoFinder\\Http\\Action\\BrowserAction") ? 0 : 1);'
...
"$repository_root/scripts/php-bin.sh" -r 'require "vendor/autoload.php"; $class = new ReflectionClass("SohoPHP\\SoFinder\\SoFinderBundle"); $package = dirname((string) $class->getFileName(), 2); $routes = SohoPHP\SoFinder\Routing\SymfonyRouteCollectionFactory::create(); exit(is_file($package . "/dist/manifest.json") && is_file($package . "/src/Resources/config/routes.php") && class_exists("SohoPHP\\SoFinder\\Symfony\\SymfonyEndpointController") && class_exists("Symfony\\Bridge\\PsrHttpMessage\\Factory\\PsrHttpFactory") && count($routes) === 52 && str_contains((string) $class->getFileName(), "/vendor/sohophp/sofinder-symfony/src/") ? 0 : 1);'
