...
export function ImageEditor({ entry, info, imageUrl, resource, watermarkUrl, presets, formats, enabledTools, maximumFileNameLength, labels, onClose, onSave }: {
  ...
  const moveWatermark = (event: { clientX: number; clientY: number }) => {
  ...
  useEffect(() => {
    const movePointer = (event: PointerEvent) => moveWatermark(event);
    const moveMouse = (event: MouseEvent) => moveWatermark(event);
