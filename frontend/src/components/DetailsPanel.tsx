...
export function DetailsPanel({ api, resource, selectedEntries, selected, imageInfo, metadata, showTags, previewImage, selectMode, selectAllowed, labels, formatDate, onChoose, onShare, pluginActions }: {
  api: Api;
  resource: string;
  ...
  labels: { details: string; selected: string; type: string; folder: string; file: string; size: string; dimensions: string; modified: string; location: string; select: string; download: string; share: string; unsupportedWebImage: string };
  ...
  return <aside className="sf-details">
    ...
    {selectedEntries.length > 1 ? <div className="sf-state">{selectedEntries.length} {labels.selected}</div> : selected ? <>
      ...
      {!selected.directory && <div className="sf-detail-actions"><a className="sf-download" href={selected.url || api.downloadUrl(resource, selected.path)} target="_blank" rel="noopener noreferrer">{labels.download}</a><button type="button" onClick={() => onShare(selected)}>{labels.share}</button></div>}
