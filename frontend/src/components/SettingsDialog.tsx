...
export function SettingsDialog({ resource, tools, features, columns, viewSizes, folderTreePlacement, quickAccessScope, availability, scale, uploadConflictStrategy, translate, onToolChange, onFeatureChange, onColumnChange, onViewSizeChange, onFolderTreePlacementChange, onQuickAccessScopeChange, onScaleChange, onUploadConflictStrategyChange, onReset, onClose }: {
  ...
  const quickAccessLabels = documentLanguage === "zh-tw" ? { title: "快速存取顯示範圍", all: "全部根目錄", resource: "目前根目錄" } : documentLanguage.startsWith("zh") ? { title: "快速访问显示范围", all: "全部根目录", resource: "当前根目录" } : { title: "Quick access scope", all: "All storage roots", resource: "Current storage root" };
  const sidebarLabels = documentLanguage === "zh-tw" ? { title: "側邊欄內容", favorites: "顯示收藏夾", quick: "顯示快速存取", files: "快速存取包含檔案" } : documentLanguage.startsWith("zh") ? { title: "侧边栏内容", favorites: "显示收藏夹", quick: "显示快速访问", files: "快速访问包含文件" } : { title: "Sidebar content", favorites: "Show Favorites", quick: "Show Quick access", files: "Include files in Quick access" };
