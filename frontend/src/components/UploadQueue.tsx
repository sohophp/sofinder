...
export function UploadQueue({ tasks, collapsed, labels, onToggle, onCancel, onCancelAll, onClearFinished, onRemove }: {
  ...
  return <section className={`sf-upload-panel${collapsed ? " collapsed" : ""}`} aria-label={labels.title}>
    <header><button className="sf-upload-collapse" onClick={onToggle} aria-expanded={!collapsed} title={collapsed ? labels.expand : labels.collapse}>{collapsed ? "›" : "⌄"}</button><strong>{labels.title}</strong><span>{finished}/{tasks.length}</span><button onClick={onCancelAll} disabled={!active}>{labels.cancelAll}</button><button onClick={onClearFinished}>{labels.clearFinished}</button></header>
    {!collapsed && <div className="sf-upload-list">{tasks.map(task => <div className={`sf-upload-task ${task.status}`} key={task.id}>
      ...
      <button className="sf-upload-remove" onClick={() => onRemove(task.id)} title={labels.remove} aria-label={`${labels.remove}: ${task.name}`}>×</button>
