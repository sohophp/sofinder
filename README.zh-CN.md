...
当前完整支持的宿主集成是 Symfony；同时提供已测试的、不依赖框架 Request／Container
的 Registry 与 `FileManager` headless 入口。Laravel 及共用的 Slim／Mezzio PSR-7
桥接会在 Symfony／PHP 8.2～8.5 基线稳定后推进。准确支持级别见
