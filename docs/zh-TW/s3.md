...
AWS S3 請省略 `endpoint`。Cloudflare R2 使用 `region: auto` 與帳戶 endpoint。MinIO 通常需要 `use_path_style_endpoint: true`；除非在受信任的開發網路明確設定 `allow_insecure_endpoint: true`，否則 HTTP 會遭拒絕。設定 `root: /` 可公開 bucket 根目錄。
