...
每週執行的 **Storage provider compatibility** 工作流程會對全新 MinIO 服務執行公開 Adapter 契約；當儲存庫完整設定對應 Secret 時，也會檢查 AWS S3、Cloudflare R2 與 Backblaze B2。外部工作必須使用專用非正式 bucket 及最小權限憑證。
版本化 bucket 還須允許列出及刪除物件版本，以確保隨機契約前綴可完整清除。
