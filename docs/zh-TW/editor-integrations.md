...
Adapter 呼叫 CKEditor 5 公開的 `insertImage` 與替代文字 Command。啟用資產目錄後，Picker 與上傳會保留明確的 `alt=""`，優先使用資產預設替代文字；未設定時才使用移除副檔名的檔名，並盡量寫入響應式變體、寬高及 `data-sofinder-asset-id`。安裝與授權要求請參考
