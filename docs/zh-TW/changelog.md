...
- 穩定套件觀察期間對 Packagist 公告介面的瞬時失敗進行有限重試，同時保留真實漏洞失敗。
...
- 將不可變的 1.0 缺陷觀察證據與乾淨 Consumer 巡檢分離；由 Packagist 安裝前，先確認
  最新穩定版本已存在於全部七個同步 Repository。
- 為私有前端 Workspace 使用明確的非發佈版本，避免其內部 Metadata 與 Composer
  Release 繼續漂移。

## 1.2.1 - 2026-08-30

- 將上傳進度改為可拖曳、可關閉的獨立視窗，並可由工具列百分比恢復；檔案與資料夾
  上傳合併為一個分割按鈕，預設執行檔案上傳。
- 將資源回收筒移至左側欄，為最近使用新增可搜尋的獨立頁面，並將最近使用與收藏檔案
  的側欄預覽限制為五項。
- 改善桌面與行動版分頁，使用精簡圖示按鈕、穩定頁碼指示及更清楚的每頁數量輸入；
  英文介面統一使用 Trash。

## 1.2.0 - 2026-08-30

- Core、HTTP、PSR-15、S3 和 Symfony 6.4 Bridge 新增 PHP 8.1 支援，同時
  Symfony 7.4 與 Laravel 12／13 繼續以 PHP 8.2 為最低版本。
- 在相容 PHP 8.1 語法的同時保留唯讀屬性語意，並新增 PHP 8.1 相依套件解析與
  Runtime 測試覆蓋。

## 1.1.3 - 2026-08-30

- 重新整理 Symfony、Laravel、Slim、Mezzio、純 PHP PSR-15 與 Core-only
  安裝指南，並將公開安裝指令更新至目前 1.1 系列。
- 新增包含真實產品截圖的三語 CMS 編輯器指南，並從文件首頁、導覽、頁尾與
  安裝後指引提供入口。
- 更新三份 Repository README，說明 Framework 對應的 Package 選擇並展示真實
  介面；文件截圖不會進入 Composer 發佈 Archive。
...
- 將框架無關的文件預覽 Message 轉換為可自行處理的 Laravel Queue Job，使
  `document_preview.mode=auto|messenger` 實際使用 Host Bus；共用 Message DTO 已移入 Core。
...
- 將 Framework 晉級版本視為穩定最低版本而非完全相同的 Tag，使政策可如實記錄 1.0.2，
