...
AWS S3 请省略 `endpoint`。Cloudflare R2 使用 `region: auto` 与帐户 endpoint。MinIO 通常需要 `use_path_style_endpoint: true`；除非在受信任的开发网络明确配置 `allow_insecure_endpoint: true`，否则 HTTP 会遭拒绝。配置 `root: /` 可公开 bucket 根目录。
