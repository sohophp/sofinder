...
锁定在该资源。如果直接 Picker URL 只应把资源作为初始位置，请设置
`picker.lock_resource: false`；单次 SDK 调用则传入 `lockResource: false`。
存储文件和元数据无需迁移。

## 从 1.1.0 升级至 1.1.1

请完整部署重新构建的 `dist/`。复制了 Symfony S3 示例同源 `entry_url` 配置的应用，
应设置 `absolute: false`，避免将代理层误判的协议固化到编辑器内容中。仅当宿主路由确实
需要公开绝对地址，且可信代理协议配置已经验证时，才保留 `absolute: true`。

## 从 0.1.0-beta.26 升级至 0.1.0-beta.31
...
Beta.30 新增 `sofinder-wangeditor.js` 选择与上传桥接，不涉及后端或存储数据变更。请完整部署
重新构建的 `dist/`，确保新入口与带哈希的共享适配器分块保持同步。

Beta.31 新增 `sofinder-jodit.js` 选择与上传桥接，不涉及后端或存储数据变更。请完整部署
重新构建的 `dist/`，确保新入口与带哈希的共享适配器分块保持同步。

## 从 0.1.0-beta.25 升级至 0.1.0-beta.26

请完整部署重新构建的 `dist/`。CKEditor 5 集成可继续把
`createCkeditor5UploadPlugin(options)` 直接放入 `plugins` 或 `extraPlugins`；工厂现在
返回新版 CKEditor 所需的可构造插件。请移除宿主中手动调用旧 Adapter 函数的包装类。
无需迁移存储文件或后端配置。

## 从 0.1.0-beta.24 升级至 0.1.0-beta.25

请完整部署重新构建的 `dist/`，使选择操作菜单不再被横向滚动工具栏遮挡。无需迁移
文件、metadata、配置或公开 API。自行托管文档时应同时重新构建全部语言，以确保导航和
上下页链接保持当前语言。

## 从 0.1.0-beta.23 升级至 0.1.0-beta.24

请完整部署重新构建的 `dist/`；既有文件保持兼容，也不会自动扫描整个存储。资产搜索默认
启用并执行有数量上限的授权扫描，大型项目应接入索引式 `AssetSearchProviderInterface`。
`asset_usage` 与 `asset_access_sessions` 继续默认关闭；多节点启用后会使用共享原子状态，
也可配置自定义共享 Store。已有资产可继续懒注册，或先运行
`sofinder:assets:migrate --dry-run`，确认后再执行正式迁移。私有访问会话 URL 是短期
Bearer 凭据，不能永久写入编辑器内容或宿主日志。启用使用关系登记的宿主应在删除前执行
预检，同时继续应用自身业务授权。
