...
资产采用懒注册随机 UUID；重命名、移动、覆盖和回收站恢复保留 ID，上传及复制创建新 ID。集群状态启用时自动采用共享目录。图片变体只接受白名单尺寸和格式、不放大，并继承资源鉴权。Workspace 必须由宿主可信的 `WorkspaceResolverInterface` 从登录上下文解析，不能直接信任查询参数；真正的存储隔离仍由宿主资源映射保证。
