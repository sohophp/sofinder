...
- 稳定包观察期间对 Packagist 公告接口的瞬时失败进行有限重试，同时保持真实漏洞失败。

## 1.3.0 - 2026-09-04

- 新增公开、框架无关的存储 Adapter 契约验证器，以及每周 MinIO 与按 Secret
  启用的 AWS S3、Cloudflare R2、Backblaze B2 供应商检查。
- 固定 Symfony 集成演示中的全部第三方运行时版本，并新增要求六个真实编辑器实例
  完成初始化的每周浏览器契约。

## 1.2.2 - 2026-09-04

- 将不可变的 1.0 缺陷观察证据与干净消费者巡检分离；从 Packagist 安装前，先确认最新
  稳定版本已存在于全部七个同步仓库。
- 为私有前端 Workspace 使用明确的非发布版本，避免其内部 Metadata 与 Composer
  Release 继续漂移。

## 1.2.1 - 2026-08-30

- 将上传进度改为可拖动、可关闭的独立窗口，并可从工具栏百分比恢复；文件与文件夹
  上传合并为一个拆分按钮，默认执行文件上传。
- 将回收站移至左侧栏，为最近使用增加可搜索的独立页面，并将最近使用和收藏文件的
  侧栏预览限制为五项。
- 优化桌面与移动端分页，使用紧凑图标按钮、稳定页码指示和更清晰的每页数量输入；
  英文界面统一使用 Trash。
...
- Picker 显式传入 `resource` 时，默认将导航与返回结果锁定在该资源，并在启动时
  验证资源可见性；SDK 会拒绝资源不匹配的弹窗结果。
- 宿主可通过 `picker.lock_resource: false` 设置直接 Picker URL 的全局默认行为，
  或在单次 SDK 调用中传入 `lockResource: false` 以允许显示和切换其他授权资源。
- 保留 Laravel 和 PSR-15 拆分仓库的连续生成历史，同时继续拒绝发布任何非生成的
  远端变更。
...
- Symfony S3 示例的同源宿主文件路由改为相对 URL，避免编辑器从子目录选择图片时，
  将请求上下文误判的 HTTP／HTTPS 协议固化到内容中。
- 让编辑器演示的大文件上传兼容 PHP 常见的 2 MiB 开发限制；普通表单上传失败不再产生
  未处理 Promise；同时恢复演示空间配额显示及支持资源的工具栏回收站入口。

## 1.1.0 - 2026-08-30

- 将 Laravel 12／13，以及供 Slim 4、Mezzio 3 和纯 PHP 使用的共享 PSR-15 Bridge
  晋级为正式支持；如实记录维护者批准的观察期豁免，同时保留成功的在线 CI、零 P0/P1
  审计、拆分发布及干净消费者证据。
- 创建公开的 `sohophp/sofinder-laravel` 与 `sohophp/sofinder-psr15` 拆分仓库，
  用于同步 1.1 版本线。
- 新增框架无关的文档预览 Dispatcher Contract。Symfony、Laravel 与 PSR-15 现在共享
  同一显式异步预览边界；PSR 宿主选择 Messenger 模式但未提供 Dispatcher 时会启动失败。
